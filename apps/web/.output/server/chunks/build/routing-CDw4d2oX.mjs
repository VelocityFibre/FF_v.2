...
function ze(t, e, n, r = {}) {
  const { signal: [s, o], utils: a = {} } = t, c = a.parsePath || ((i) => i), f = a.renderPath || ((i) => i), h = a.beforeLeave || ge(), l = W("", r.base || "");
  ...
  }, [v, C] = createSignal(s().value), [S, ne] = createSignal(s().state), O = Fe(v, S, a.queryWrapper), F = [], U = createSignal(isServer ? ce() : []), z = createMemo(() => typeof r.transformUrl == "function" ? I(e(), r.transformUrl(O.pathname)) : I(e(), O.pathname)), H = () => {
    const i = z(), u = {};
  ...
  return createRenderEffect(on(s, (i) => y("native", i), { defer: true })), { base: K, location: O, params: re, isRouting: m, renderPath: f, parsePath: c, navigatorFactory: oe, matches: z, beforeLeave: h, preloadRoute: ie, singleFlight: r.singleFlight === void 0 ? true : r.singleFlight, submissions: U };
  function se(i, u, g) {
    untrack(() => {
      if (typeof u == "number") {
      ...
      if (A !== N || b !== S()) if (isServer) {
        ...
        T && (T.response = { status: 302, headers: new Headers({ Location: A }) }), o({ value: A, replace: j, scroll: B, state: b });
      } else h.confirm(A, g) && (F.push({ value: N, replace: j, scroll: B, state: S() }), y("navigate", { value: A, state: b }));
    });
  }
  function oe(i) {
  ...
  function ie(i, u) {
    ...
    for (let j in g) {
      ...
      const { preload: b } = x;
      u && b && runWithOwner(n(), () => b({ params: B, location: { pathname: i.pathname, search: i.search, hash: i.hash, query: V(i), state: null, key: "" }, intent: "preload" }));
    }
    R = P;
  }
  function ce() {
...
export { $e as $, Ce as C, De as D, E, He as H, Ie as I, Me as M, Oe as O, Q, Ue as U, We as W, _e as _, I as a, ee as e, ge as g, qe as q, ve as v, ze as z };
//# sourceMappingURL=routing-CDw4d2oX.mjs.map
